...
def create_result_error(command, curr_dir): //not yet took into consideration
...
def create_success(FS, curr_dir, command):
    return (True, FS, curr_dir, command) //not yet taken into consideration
...
def tief_copy(item):
    ...
    for i in item:
        new_list.append(tief_copy(i))

    return new_list

def rm_etwas(FS, path):
    for i in range(len(FS) - 1, 1, -1):
        if FS[i][0] == path[0]:
            if len(path) == 1:
                del FS[i]
            else:
                FS[i] = rm_etwas(FS[i], path[1:])

    return FS


def try_cd(FS, curr_dir, command, parameters):
...
def try_rmdir(FS, curr_dir, command, parameters):
...
def try_rm(FS, curr_dir, command, parameters):
    if len(parameters) != 1: return create_error(FS, curr_dir, command)
...
def try_mkdir(FS, curr_dir, command, parameters):
    if len(parameters) != 1: return create_error(FS, curr_dir, command)
...
def try_exec(FS, curr_dir, command, parameters):
    if len(parameters) != 1: return create_error(FS, curr_dir, command)
...
def try_cp(FS, curr_dir, command, parameters):
    if len(parameters) != 2: return create_error(FS, curr_dir, command)

    start = search_path(FS, parameters[0])
    target = search_path(FS, parameters[1])

    if start is None: return create_error(FS, curr_dir, command)

    copy_start = tief_copy(source)
...
def try_command(FS, curr_dir, command):# main command check
    ...
    if command_name == 'cd':
        return try_cd(FS, curr_dir, command, parameters)
    elif command_name == 'rmdir':
        return try_rmdir(FS, curr_dir, command, parameters)
    elif command_name == 'rm':
        return try_rm(FS, curr_dir, command, parameters)
    elif command_name == 'mkdir':
        return try_mkdir(FS, curr_dir, command, parameters)
    elif command_name == 'cp':
        return try_cp(FS, curr_dir, command, parameters)
    elif command_name == 'exec':
        return try_exec(FS, curr_dir, command, parameters)

    return create_error(FS, curr_dir, command)

def check_commands(FS, command_list):  ##checking all commands
    curr_dir = []

    for command in command_list:
        res = try_command(FS, curr_dir, command)
